...
export const getServerSideProps = async (context) => {
  ...
  const res = await axios.get(`http://www.notiefi.com/api/classes/${id}`);
  const classs = await res.data;

  const res2 = await axios.get(`http://www.notiefi.com/api/users/user/classes`, {
    headers: {
  ...
  const res3 = await axios.get(`http://www.notiefi.com/api/users/user/likes`, {
    headers: {
      Cookie: context.req.headers.cookie,
    },
  });
